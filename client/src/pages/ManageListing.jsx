...
const ManageListing = () => {
  ...
  const [loadingListings, setLoadingListing] = useState(false)
        ...
        <form onSubmit={handleSubmit} className='space-y-8'>
            ...
            <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
              <InputField label='Listing Title *' value={formData.title}
                ...
                onChange={(v) => handleInputChange('title', v)}
                 ...
                 <SelectField label='Platform*' options={platforms} value={formData.
                  platform} onchange={(v) => handleInputChange('platform', v)}
                  required={true} />

                  <InputField label='Username/Handle *' value={formData.username}
                  placeholder='@username' onchange={(v)=> handleInputChange
                    ('username', v)} required={true} />

                    <SelectField label='Niche/Category *' options={niches} value={formData.
                  niche} onchange={(v) => handleInputChange('niche', v)}
                  required={true} />
            </div>
          </Section>
            ...
            <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-6'>
              ...
              <InputField label='Followers Count *' type='number' min={0} 
               value={formData.followers_count}
                  placeholder='10000' onchange={(v) => handleInputChange
                    ('followers_count', v)} required={true} />

                    <InputField label='Engagement Rate (%)' type='number' min={0} max={100}
                    value={formData.engagement_rate}
                  placeholder='4' onchange={(v)=> handleInputChange
                    ('engagement_rate', v)} required={true} />

                    <InputField label='Monthly Views/Impressions'  type='number' min={0}
                    value={formData.monthly_views}
                  placeholder='100000' onchange={(v)=> handleInputChange
                    ('monthly_views', v)} required={true} />
            </div>

            <div className='grid grid-cols-1 md:grid-cols-2 gap-6 mb-6'>

              <InputField label='Primary Audience Country' value={formData.country}
              placeholder='United States' onchange={(v) =>
                handleInputChange('country', v)} />

                 <SelectField label='Primary Audience Age Range' options={ageRanges}
                  value={formData.age_range} onchange={(v) => handleInputChange('age_range', v)}/>
          ...
          <Section title='Pricing & Description'>
            ...
            value={formData.price} placeholder='2500.00' onChange={(v)=> 
              handleInputChange('price', v)} required={true} />
          ...
          <Section title='Screenshots & Proof'>
            ...
            text-center'>
              ...
              rounded-lg hover:bg-gray-50 cursor-pointer'>Choose Files
              </label>
              <p className='text-sm text-gray-500 mt-2'>Upload screenshots or proof of account analytics
              </p>
            </div>
